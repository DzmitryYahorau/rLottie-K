...
public class DispatchQueuePoolBackground {
    ...
    private Runnable cleanupRunnable = new Runnable() {
        ...
        public void run() {
            ...
            if (!queues.isEmpty() || !busyQueues.isEmpty()) {
                AndroidUtilities.globalQueue.postRunnable(this, 30000);
    ...
    private void execute(ArrayList<Runnable> runnables) {
        for (int i = 0; i < runnables.size(); i++) {
            ...
            if (!busyQueues.isEmpty() && (totalTasksCount / 2 <= busyQueues.size() || queues.isEmpty() && createdCount >= maxCount)) {
                ...
                queue = new DispatchQueue(THREAD_PREFIX + guid + "_" + Random.Default.nextInt());
            ...
            if (!cleanupScheduled) {
                AndroidUtilities.globalQueue.postRunnable(cleanupRunnable, 30000);
                cleanupScheduled = true;
            }
            totalTasksCount++;
            ...
            if (queue.getPriority() != Thread.MAX_PRIORITY) {
                queue.setPriority(Thread.MAX_PRIORITY);
            }
            queue.postRunnable(() -> {
                runnable.run();
    ...
    public static void execute(Runnable runnable, boolean now) {
        if (Thread.currentThread() != ApplicationLoader.Companion.getApplicationHandler().getLooper().getThread()) {
            Log.e("rLottie", "wrong thread");
