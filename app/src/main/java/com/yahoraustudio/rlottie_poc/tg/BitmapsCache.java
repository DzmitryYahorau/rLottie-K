package com.yahoraustudio.rlottie_poc.tg;

import android.graphics.Bitmap;
import android.graphics.BitmapFactory;
import android.os.Build;

import com.yahoraustudio.rlottie_poc.ApplicationLoader;
import com.yahoraustudio.rlottie_poc.DispatchQueuePoolBackground;

import com.yahoraustudio.rlottie_poc.AndroidUtilities;
import com.yahoraustudio.rlottie_poc.RLottieDrawable;
...
public class BitmapsCache {
    ...
    private final static int N = AndroidUtilities.clamp(Runtime.getRuntime().availableProcessors() - 2, 6, 1);
    ...
    public BitmapsCache(File sourceFile, Cacheable source, CacheOptions options, int w, int h, boolean noLimit) {
        ...
        // todo think about it, maybe better to use custom dir
        File cacheDir = ApplicationLoader.Companion.getApplicationLoaderInstance().getCacheDir();

        File fileTmo = new File(cacheDir, "acache");
        if (!mkdir) {
            fileTmo.mkdir();
            mkdir = true;
        }
...
//        SharedConfig.getDevicePerformanceClass() >= SharedConfig.PERFORMANCE_CLASS_HIGH
        if (true) { //todo: device performance would be high by default
...
//        cancelled.set(true);
    ...
    public int getFrame(int index, Bitmap bitmap) {
        ...
        try {
            ...
            index = AndroidUtilities.clamp(index, frameOffsets.size() - 1, 0);
