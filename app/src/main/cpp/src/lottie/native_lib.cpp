////
//// Created by Dzmitry Yahorau on 26.04.2025.
////
#include <jni.h>
#include <string>
...
Java_com_yahoraustudio_rlottie_1poc_MainActivity_stringFromJNI(JNIEnv *env, jobject thiz) {
    std::string hello = "Hello from C++!";
    return env->NewStringUTF(hello.c_str());
}

JNIEXPORT jlong
Java_com_yahoraustudio_rlottie_1poc_RLottieDrawable_create(JNIEnv *env, jclass clazz, jstring src,
                                                           jstring json, jint w, jint h,
                                                           jintArray data, jboolean precache,
                                                           jintArray colorReplacement,
                                                           jboolean limitFps, jint fitzModifier) {
...
JNIEXPORT jlong
Java_com_yahoraustudio_rlottie_1poc_RLottieDrawable_getFramesCount(JNIEnv *env, jclass clazz,
                                                                   jstring src, jstring json) {
    auto info = new LottieInfo();
    char const *srcString = env->GetStringUTFChars(src, nullptr);
    info->path = srcString;
    if (json != nullptr) {
        char const *jsonString = env->GetStringUTFChars(json, nullptr);
        if (jsonString) {
            info->animation = rlottie::Animation::loadFromData(jsonString, info->path, nullptr,
                                                               FitzModifier::None);
            env->ReleaseStringUTFChars(json, jsonString);
        }
    } else {
...
JNIEXPORT jdouble
Java_com_yahoraustudio_rlottie_1poc_RLottieDrawable_getDuration(JNIEnv *env, jclass clazz,
                                                                jstring src, jstring json) {
    auto info = new LottieInfo();
    char const *srcString = env->GetStringUTFChars(src, nullptr);
    info->path = srcString;
...
JNIEXPORT jlong
Java_com_yahoraustudio_rlottie_1poc_RLottieDrawable_createWithJson(JNIEnv *env, jclass clazz,
...
JNIEXPORT void
Java_com_yahoraustudio_rlottie_1poc_RLottieDrawable_destroy(JNIEnv *env, jclass clazz, jlong ptr) {
    if (!ptr) {
        return;
    }
...
JNIEXPORT jint
Java_com_yahoraustudio_rlottie_1poc_RLottieDrawable_getFrame(JNIEnv *env, jclass clazz, jlong ptr,
                                                             jint frame, jobject bitmap, jint w,
                                                             jint h, jint stride, jboolean clear) {
